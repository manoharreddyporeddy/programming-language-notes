...
namespace null_propagator
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // bad
            var minPrice1 = p.PriceBreaks[0].Price;
            ...
            if (p != null
                && p.PriceBreaks != null
                && p.PriceBreaks[0] != null)
            {
                minPrice2 = p.PriceBreaks[0].Price;
            }

            // better - new - null propagation operator(?) - double or null
            var minPrice3 = p?.PriceBreaks?[0]?.Price;

            // best - new - null propagation operator(?) & null coalescing operator (??) - double or 0
            var minPrice4 = p?.PriceBreaks?[0]?.Price ?? 0;
